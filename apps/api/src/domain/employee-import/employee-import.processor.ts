      ...
      const { employeeId, userId } = await db.$transaction(async (tx) => {
        ...
        const employee = await tx.employee.create({
          data: {
            ...
            // Extended profile fields (SPEC-040). Validated rows carry null for
            // blanks; map to undefined so Prisma leaves the column at its default.
            ...
            socialInsuranceNumber: row.socialInsuranceNumber ?? undefined,
            healthcareFacility: row.healthcareFacility ?? undefined,
            motorbikeRegistration: row.motorbikeRegistration ?? undefined,
          },
          select: { id: true },
        });
