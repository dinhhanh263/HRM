...
import type { PurchaseRequestAttachmentDto } from '@hrm/shared';
...
interface PurchaseAttachmentUploaderProps {
  requestId: string;
...
export function PurchaseAttachmentUploader({
  requestId,
  attachments,
  editable,
  maxFiles = 10,
}: PurchaseAttachmentUploaderProps) {
  const { t } = useTranslation('purchase');
  const inputRef = useRef<HTMLInputElement>(null);
  const [downloadingId, setDownloadingId] = useState<string | null>(null);
  const uploadMutation = useUploadPurchaseAttachment();
  const deleteMutation = useDeletePurchaseAttachment();
  ...
  async function handleDownload(att: PurchaseRequestAttachmentDto) {
    setDownloadingId(att.id);
    try {
      await downloadPurchaseAttachment(requestId, att);
    } catch {
  ...
  function handleDelete(att: PurchaseRequestAttachmentDto) {
