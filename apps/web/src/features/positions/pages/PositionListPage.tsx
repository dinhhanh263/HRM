...
} from '../components/PositionFormSheet';
import {
  usePositions,
  useCreatePosition,
  useUpdatePosition,
...
export function PositionListPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(
      (p) =>
        ...
        t(getLevelKey(p.level), { level: p.level }).toLowerCase().includes(q)
    );
  }, [data, searchInput, t]);
  ...
  function handleSubmit(formData: PositionFormData) {
    const level = Number(formData.level);
    const hasDepartment = formData.departmentId !== NO_DEPARTMENT;

    if (editing) {
      updateMutation.mutate(
        {
          name: formData.name,
          departmentId: hasDepartment ? formData.departmentId : null,
          level,
        },
        {
          onSuccess: () => {
      ...
      createMutation.mutate(
        {
          name: formData.name,
          departmentId: hasDepartment ? formData.departmentId : undefined,
          level,
        },
        {
          onSuccess: () => {
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {hasEmployees
                ? t('delete.descriptionHasEmployees', {
