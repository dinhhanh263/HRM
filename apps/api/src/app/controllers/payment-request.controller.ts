...
import { paymentRequestService } from '../../domain/services/payment-request.service.js';
...
import { paymentRequestQuerySchema } from '../validators/payment-request.validator.js';
import { buildPaymentExportWorkbook } from '../../domain/payment-request/export.js';
 ...
 * route only checks payment_request:view, which every role has. SUPER_ADMIN bypasses.
 */
async function requireReviewCapability(req: Request) {
  const user = req.user!;
  if (user.role === 'SUPER_ADMIN') return;
  if (!user.roleId) {
    throw new ForbiddenError('You do not have permission to review payment requests');
  }
  const granted = await permissionService.getPermissionsForRole(user.roleId);
  if (!granted.has('payment_request:approve') && !granted.has('payment_request:reject')) {
    throw new ForbiddenError('You do not have permission to review payment requests');
  }
}
...
export const paymentRequestController = {
  async listRequests(req: Request, res: Response) {
    ...
    const { page, limit, ...input } = paymentRequestQuerySchema.parse(req.query);

    if (input.scope === 'all') {
      await requireReviewCapability(req);
      const result = await paymentRequestService.list(tenantId, '', input, { page, limit });
    ...
    if (input.scope === 'review') {
      ...
      const result = await paymentRequestService.listReview(tenantId, actor, input, { page, limit });
      res.json({ success: true, data: result });
      return;
    }
    ...
    const result = await paymentRequestService.list(tenantId, employee.id, input, { page, limit });
    res.json({ success: true, data: result });
  },

  async exportRequests(req: Request, res: Response) {
    const tenantId = req.user!.tenantId;
    const { scope, ...input } = paymentRequestQuerySchema.parse(req.query);
    ...
    const rows = await paymentRequestService.getExportRows(tenantId, effectiveScope, employeeId, actor, input);
    const buffer = await buildPaymentExportWorkbook(rows);
    ...
    res.setHeader('Content-Disposition', `attachment; filename="payment-requests-${effectiveScope}-${stamp}.xlsx"`);
    res.send(buffer);
  },
  ...
  async getStats(req: Request, res: Response) {
    ...
    const data = await paymentRequestService.getStats(tenantId, year);
    res.json({ success: true, data });
  },

  async getRequest(req: Request, res: Response) {
    const tenantId = req.user!.tenantId;
    const request = await paymentRequestService.getById(req.params.id, tenantId);
  ...
  async updateRequest(req: Request, res: Response) {
    ...
    const request = await paymentRequestService.update(req.params.id, tenantId, employee.id, req.body);
    res.json({ success: true, data: request });
  },
  ...
  async uploadAttachment(req: Request, res: Response) {
    ...
    const file = req.file;
    if (!file) {
      throw new BadRequestError('Chưa có tệp nào được tải lên (trường "file")', 'PAYMENT_NO_FILE');
    }
    const data = await paymentRequestService.addAttachment(req.params.id, tenantId, employee.id, {
      buffer: file.buffer,
  ...
  async deleteAttachment(req: Request, res: Response) {
    const tenantId = req.user!.tenantId;
    const employee = await requireCurrentEmployee(req);
    await paymentRequestService.removeAttachment(
      req.params.id,
      tenantId,
      employee.id,
      req.params.attId,
  ...
  async downloadAttachment(req: Request, res: Response) {
    ...
    const { stream, contentType, fileName } = await paymentRequestService.getDownload(
      req.params.id,
      tenantId,
      req.params.attId,
    );
    ...
    stream.on('error', (err: unknown) => {
      logger.error({ err, attachmentId: req.params.attId }, 'Payment attachment download stream failed');
  ...
  async approveRequest(req: Request, res: Response) {
    const tenantId = req.user!.tenantId;
    const actor = await buildApprovalActor(req);
    const request = await paymentRequestService.approve(req.params.id, tenantId, actor);
    res.json({ success: true, data: request });
  },
  ...
  async respondRequest(req: Request, res: Response) {
    const tenantId = req.user!.tenantId;
    const actor = await buildApprovalActor(req);
    const request = await paymentRequestService.respond(
      req.params.id,
      tenantId,
      actor,
      req.body.mode,
  ...
  async resubmitRequest(req: Request, res: Response) {
    ...
    const employee = await requireCurrentEmployee(req);
    const actor = await buildApprovalActor(req);
    const request = await paymentRequestService.resubmit(
      req.params.id,
      tenantId,
      employee.id,
      { isSuperAdmin: actor.isSuperAdmin, roleKey: actor.roleKey },
  ...
  async cancelRequest(req: Request, res: Response) {
    const tenantId = req.user!.tenantId;
    const employee = await requireCurrentEmployee(req);
    const request = await paymentRequestService.cancel(req.params.id, tenantId, employee.id);
    res.json({ success: true, data: request });
  },

  async markPaidRequest(req: Request, res: Response) {
    const tenantId = req.user!.tenantId;
    const actor = await buildApprovalActor(req);
    const request = await paymentRequestService.markPaid(
      req.params.id,
      tenantId,
      actor,
